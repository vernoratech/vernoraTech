...
const Stats = () => {
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        entries.forEach((entry) => {
          // Trigger only when the section is mostly visible in the viewport
  ...
  return (
    <section ref={sectionRef} className="relative py-24 overflow-hidden bg-[#FAFAFA]">
      {/* Background Decor */}
        ...
        <div className="-mx-6 xl:mx-0">
          <div className="no-scrollbar flex snap-x snap-mandatory gap-4 overflow-x-auto px-6 sm:pb-6 xl:hidden">
            {stats.map((stat, index) => (
              <div
                key={index}
                className={`group relative flex-shrink-0 snap-center overflow-hidden rounded-3xl border border-[#D9E4F2] bg-white p-8 shadow-sm transition-all duration-700 transform-gpu hover:-translate-y-1 hover:border-[#2DA3DB]/30 hover:shadow-xl hover:shadow-[#1A3A6F]/5 w-72 sm:w-80 ${hasAnimated ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-10'
                  }`}
                style={hasAnimated ? { transitionDelay: `${index * 80}ms` } : undefined}
              >
          ...
          <div className="hidden xl:grid xl:grid-cols-4 xl:gap-6">
            {stats.map((stat, index) => (
              <div
                key={index}
                className={`group relative overflow-hidden rounded-3xl border border-[#D9E4F2] bg-white p-8 shadow-sm transition-all duration-700 transform-gpu hover:-translate-y-1 hover:border-[#2DA3DB]/30 hover:shadow-xl hover:shadow-[#1A3A6F]/5 ${hasAnimated ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-10'
                  }`}
                style={hasAnimated ? { transitionDelay: `${index * 80}ms` } : undefined}
              >
                <div className="relative flex flex-col items-center text-center">
