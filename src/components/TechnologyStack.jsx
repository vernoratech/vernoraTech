...
const TechnologyStack = () => {
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        entries.forEach((entry) => {
          // Trigger when the section is reasonably visible in the viewport
  ...
  return (
    <section ref={sectionRef} className="relative py-12 sm:py-16 bg-[#FAFAFA] overflow-hidden">
      <style>
        {`
          @keyframes scroll {
      ...
      {/* Heading */}
      <div
        className={`relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-16 text-center transform-gpu transition-all duration-700 ease-out ${hasAnimated ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-6'
          }`}
      >
      ...
      {/* Marquee Rows */}
      <div
        className={`space-y-6 sm:space-y-10 transform-gpu transition-all duration-700 ease-out ${hasAnimated ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-4'
          }`}
        style={hasAnimated ? { transitionDelay: '150ms' } : undefined}
      >
        {/* Row 1: Frontend */}
        <div className="relative">
          <div className="absolute left-1/2 -translate-x-1/2 -top-5 z-10">
            <span className="bg-[#FAFAFA] px-2 text-[9px] sm:text-[10px] font-bold uppercase tracking-widest text-[#6E7787]">
              Frontend
            </span>
          </div>
          <MarqueeRow
      ...
      {selectedTech && (
        <div
          className={`relative max-w-4xl mx-auto mt-12 sm:mt-16 px-4 sm:px-6 lg:px-0 transform-gpu transition-all duration-700 ease-out ${hasAnimated ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-4'
            }`}
          style={hasAnimated ? { transitionDelay: '250ms' } : undefined}
        >
          <div className="absolute inset-0 rounded-3xl bg-gradient-to-r from-[#E8F1FC] via-white to-[#FAFAFA] blur-2xl" aria-hidden="true" />
          <div className="relative rounded-3xl border border-[#D9E4F0] bg-white/90 backdrop-blur p-6 sm:p-8 shadow-xl shadow-[#0F172A]/5">
            ...
            <div className="flex flex-col gap-6">
              <div className="flex flex-col sm:flex-row items-center sm:items-start gap-4 sm:gap-4 text-center sm:text-left">
                <div
                  className={`flex h-14 w-14 sm:h-16 sm:w-16 items-center justify-center rounded-2xl bg-[#F0F7FF] ${selectedTech.color || 'text-[#1A3A6F]'
                    }`}
                >
                ...
                <div className="w-full">
                  ...
                  <p className="mt-2 text-sm text-[#4B5563] max-w-2xl mx-auto sm:mx-0">{selectedTech.summary}</p>
                </div>
              </div>
              ...
              <div className="grid gap-5 sm:gap-6 md:grid-cols-2">
                ...
                <div className="rounded-2xl border border-[#E2E8F0] bg-white p-4 sm:p-5 shadow-sm text-left">
                  ...
                  <p className="text-[#1C1F26] text-xs sm:text-sm leading-relaxed">{selectedTech.delivery}</p>
                </div>
              </div>
            </div>
          </div>
