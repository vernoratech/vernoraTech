...
function App() {
  ...
  React.useEffect(() => {
    ...
    const isExpired =
      !hasSeen || Number.isNaN(timestamp) || now - timestamp > twentyFourHours;
  ...
  return (
    <div className="App">
      ...
      <Routes>
        ...
        <Route
          path="/blog"
          element={<BlogInsights onReadBlog={navigateToBlog} />}
        />
        <Route path="/blog/:blogId" element={<BlogDetailRoute />} />
        <Route
          path="/portfolio"
          element={<PortfolioPage onBackToHome={navigateToHome} />}
        />
        <Route
          path="/case-studies"
          element={<CaseStudiesPage onBackToHome={navigateToHome} />}
        />
        ...
        <Route
          ...
          element={<Business_Website_Development />}
        />
        <Route
          path="/services/ecommerce"
          element={<E_Commerce_Store_Setup />}
        />
        <Route
          path="/services/landing-pages"
          element={<Landing_Page_For_Ads />}
        />
        <Route
          path="/services/booking-system"
          element={<Booking_Appointment_System />}
        />
        <Route
          path="/services/portfolio-sites"
          element={<Portfolio_Personal_Branding_Sites />}
        />
        <Route path="/services/speed" element={<Fix_Slow_Website />} />
        <Route
          path="/services/bug-fixes"
          element={<Bug_Fixes_Feature_Enhancements />}
        />
        <Route
        ...
        <Route
          path="/services/inventory-system"
          element={<Inventory_Order_Tracking_System />}
        />
        <Route
          path="/services/invoice-tool"
          element={<Invoice_Billing_Generator />}
        />
        <Route
          path="/services/mini-crm"
          element={<Client_Management_System />}
        />
        <Route
          path="/services/timesheet"
          element={<Staff_Attendance_Timesheet />}
        />
        <Route
          path="/services/dashboard"
          element={<Custom_Dashboard_Reports />}
        />
        <Route
          path="/services/maintenance"
          element={<Monthly_Website_Maintenance />}
        />
        <Route
          path="/services/hosting-fix"
          element={<Fix_Hosting_Issues_Migration />}
        />
        <Route
          path="/services/security"
          element={<Malware_Cleanup_Security_Hardening />}
        />
        <Route path="/services/fixes" element={<Broken_Pages_Layout_Fixes />} />
        <Route
          path="/services/support"
          element={<Small_Tasks_Support_Plan />}
        />
        <Route
          path="/services/upgrades"
          element={<Plugin_Library_Upgrades />}
        />
        <Route
          path="/services/restore"
          element={<Emergency_Restore_Support />}
        />
        <Route
          path="/services/small-changes"
          element={<Small_Monthly_Changes />}
        />
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center pointer-events-none">
          ...
          <div className="relative bg-white rounded-lg shadow-xl w-11/12 max-w-2xl p-6 pointer-events-auto">
            <div className="flex items-start justify-between mb-4">
              <div>
                <h4 className="text-xl font-semibold text-gray-900">
                  {activePlan.name} — Terms & Conditions
                </h4>
            ...
            <div className="space-y-4 max-h-[60vh] overflow-y-auto pr-1">
              {activePlan.terms &&
                activePlan.terms.map((section, i) => (
                  <div key={i}>
            ...
            <div className="mt-6 flex items-center justify-end gap-3">
              <button className="btn-secondary" onClick={closeTerms}>
                Close
              </button>
              <a href="#contact" className="btn-primary" onClick={goToContact}>
                Contact us
              </a>
            </div>
          </div>
      ...
      {showCookieConsent && (
        <div
          className={`fixed bottom-0 left-0 right-0 bg-[#1C1F26] text-white z-50 shadow-lg transform transition-transform duration-300 ease-out ${
